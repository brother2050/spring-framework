...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		// 警告：尽管看起来可以很容易地重构此方法的主体以避免使用多个循环和多个列表，但使用多个列表和对处理器名称的多次传递是有意为之的。
		// 我们必须确保遵守 PriorityOrdered 和 Ordered 处理器的合同。
		// 具体来说，我们不能导致处理器被实例化（通过 getBean（） 调用）或以错误的顺序在 ApplicationContext 中注册。
		//
		// 在提交拉取请求 （PR） 以更改此方法之前，请查看涉及 PostProcessorRegistrationDelegate 更改的所有被拒绝的 PR 的列表，
		// 以确保您的提案不会导致重大更改：
		...
		if (beanFactory instanceof BeanDefinitionRegistry registry) {
			...
			// 不要在此处初始化 FactoryBeans：我们需要保留所有未初始化的常规 bean，以便让 bean factory 后处理器应用于它们！
			// 在实现 PriorityOrdered、Ordered 和其他部分的 BeanDefinitionRegistryPostProcessors 之间分开。
	...
	public static void registerBeanPostProcessors(
		...
		// 但使用多个列表和对处理器名称的多次传递是有意为之的。我们必须确保遵守 PriorityOrdered 和 Ordered 处理器的合同。
		// 具体来说，我们不能导致处理器被实例化（通过 getBean（） 调用）或以错误的顺序在 ApplicationContext 中注册。
		//
		// 在提交拉取请求 （PR） 以更改此方法之前，
		// 请查看涉及对 PostProcessorRegistrationDelegate 的更改的所有被拒绝的 PR 列表，
		// 以确保您的提案不会导致重大更改：
		// https://github.com/spring-projects/spring-framework/issues?q=PostProcessorRegistrationDelegate+is%3Aclosed+label%3A%22status%3A+declined%22

		String[] postProcessorNames = beanFactory.getBeanNamesForType(BeanPostProcessor.class, true, false);

		// 注册记录信息消息的 BeanPostProcessorChecker
		//当在 BeanPostProcessor 实例化期间创建 Bean 时，
		// 即，当一个 Bean 没有资格被所有 BeanPostProcessor 处理时。
		int beanProcessorTargetCount = beanFactory.getBeanPostProcessorCount() + 1 + postProcessorNames.length;
		beanFactory.addBeanPostProcessor(
				new BeanPostProcessorChecker(beanFactory, postProcessorNames, beanProcessorTargetCount));

		// 在实现 PriorityOrdered 的 BeanPostProcessor 之间分开，
		// Ordered, and the rest.
